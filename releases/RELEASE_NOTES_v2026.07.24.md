...
手机浏览器中的豆包输入法、微信输入法或其他手机输入法，可将语音/文字输入经可信局域网发送到 Windows 当前输入框。支持托盘常驻、二维码访问和 Token 校验。
...
普通设置页切换第三方输入法时，建议使用雷电 `9.0.44`（Android 9）配合 DirectADB。另有一套已经实测成功的雷电 `9.0.47 + Root` 方案：分辨率 `1080 × 1920`、DPI `480`；小键盘 `-` 的参考坐标为 `770,1195`，小键盘 `+` 的参考坐标为 `965,1740`。完整教程说明了 Root 切换命令与坐标校准方法。

- [雷电官方输入法切换教程](https://help.ldmnq.com/docs/ru-he-zai-mo-ni-qi-nei-qie-huan-shu-ru-fa)
- [雷电 9.0.44 官方安装包](https://res.ldmnq.com/download/leidian9/ldinst_9.0.44.exe)
- [本项目完整配置教程](https://github.com/sjlsl6/-/blob/main/docs/LDPLAYER_DOUBAO.md)
