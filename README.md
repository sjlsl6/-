...
把电脑上的 ChatGPT、DeepSeek、豆包或其他 AI 输入框选中后，在手机或安卓模拟器中说话，再点击手机输入法右下角的“换行”或“确定”，AirType 会把文字写入电脑当前输入框，并自动模拟一次 Windows 回车完成发送。适合连续进行 AI 语音对话。
...
三个 EXE 都是托盘程序，双击启动后不会出现传统主界面，而是隐藏到 Windows 任务栏右下角的系统托盘。没有看到图标时，请点击任务栏右下角的“显示隐藏的图标”箭头。

- **右键托盘图标**：打开程序功能菜单。
- **双击 AirType 托盘图标**：查看运行状态。
- **双击 Wireless 或 DirectADB 托盘图标**：在浏览器中打开本机设置页面。

因此，双击 EXE 后没有弹出窗口并不代表程序没有启动。Wireless 与 DirectADB 的设置界面虽然显示在浏览器中，但它们是电脑本机页面，不是需要注册或上传配置的云端网站。

## 三种推荐用法

1. **只用 AirType**：手机和电脑位于同一可信局域网，手机扫码打开输入网页即可，不需要 ADB。
2. **AirType + Wireless**：适合支持无线调试的 Android 主力机。AirType 负责把文字发送到电脑，Wireless 负责用数字小键盘点击手机上的语音、确定等固定位置。
3. **AirType + DirectADB**：适合雷电模拟器、网易 MuMu 模拟器、其他已接入 ADB 的模拟器或 USB 调试真机。

ADB Tap 的坐标不会自动适配所有设备。请按照自己的手机/模拟器分辨率和输入法界面，手动测量并保存 `-` / `+` 点击坐标。

## AirType：手机语音/文字输入电脑

工作链路：`手机输入法/语音输入 → 手机网页 → 局域网 → Windows → 当前输入框`

1. 双击 `QAA-AirType-Official-Tray-v2.3.0.exe`。
2. 右键任务栏右下角的 AirType 托盘图标，选择“显示手机二维码”或“复制手机网址”。
3. 确保手机和电脑位于同一可信局域网，再用手机浏览器访问。
4. 先让电脑上的目标文本框获得焦点。
5. 在手机网页中使用豆包输入法、微信输入法或其他手机输入法完成语音转文字。
6. 点击手机输入法右下角的“换行”或“确定”，网页会把这个动作识别为“发送本轮”；需要在文字中真正换行时，请使用网页上的“插入换行”按钮。
7. Windows 收到文字后，会先粘贴到当前输入框，再自动模拟按下一次回车完成发送。
...
1. 让手机和电脑连接同一个局域网。
2. 在手机中开启开发者选项和“无线调试”。不同品牌手机的菜单位置可能略有差异。
3. 双击 `QAA-ADB-Tap-v1.1.0.exe`；程序会隐藏到系统托盘。
4. 右键 Wireless 托盘图标，选择“二维码配对手机”。电脑会在浏览器中显示配对二维码。
5. 在手机的“无线调试”页面选择“使用二维码配对设备”或名称相近的选项，扫描电脑上的二维码。
6. 二维码两分钟内有效。扫码后程序会自动完成配对、发现无线调试连接地址并建立点击通道。
7. 双击 Wireless 托盘图标打开设置页面，手动填写并测试三套模式中的 `-` / `+` 点击坐标。
8. 保存后使用数字小键盘 `-` 或 `+`，通过常驻 ADB Shell 触发 Android 点击。程序无需保持在前台。

无线配对码、ADB 地址和设备信息只应保留在本机。

## DirectADB：模拟器或 USB 真机

1. 启动雷电模拟器、网易 MuMu 模拟器或其他目标设备，并在模拟器设置中开启 ADB 调试；USB 真机则需开启并授权 USB 调试。
2. 双击 `QAA-ADB-Tap-v1.2.0-DirectADB.exe`；程序会隐藏到系统托盘。
3. 右键 DirectADB 托盘图标，选择“检测并连接 ADB 设备”。
4. 只有一个 `device` 状态设备时会直接使用；有多个设备时，双击托盘图标打开设置页面并选择目标设备。
5. 如果程序提示未找到 ADB，可在设置页面点击“一次性准备 ADB”，完成后再次检测。
6. 手动填写并测试三套模式中的 `-` / `+` 点击坐标。保存后即可在其他电脑窗口中使用全局热键。
...
- 数字小键盘 `+`：设置为手机或模拟器输入法右下角的“换行”或“确定”按钮坐标。

实际操作链路如下：

1. 在电脑上按数字小键盘 `-`，ADB Tap 点击手机/模拟器的语音输入按钮。
2. 说完文字后按数字小键盘 `+`，ADB Tap 点击手机输入法的“换行”或“确定”按钮。
3. AirType 手机网页把这个按键动作识别为“发送本轮”，将文字传到 Windows。
...
豆包输入法和微信输入法都可以作为 AirType 的手机端输入法。ADB Tap 只负责点击坐标，不读取输入法内容；只要输入法能够在 AirType 网页输入文字，并通过右下角的“换行”或“确定”触发网页发送，就可以使用这套组合方式。
...
AirType 参考/延续了 [QAA-Tools/qaa-airtype](https://github.com/QAA-Tools/qaa-airtype) 的使用目标。上游采用 MIT License，版权声明为 `Copyright (c) 2025 Sync Input Contributors`；本仓库保留该声明与 MIT 许可要求，并明确属于社区修改/扩展版本。Wireless 与 DirectADB 是本项目根据实际使用需求设计，并借助 ChatGPT 编写、测试和完善的两个工具。
